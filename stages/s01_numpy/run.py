...
Run via `uv run python -m stages.s01_numpy.run`. Same arguments and
output shape as stage 00 — replaceable in any Dagster asset, identical
on-disk Zarr.
"""
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Stage 01: vectorised-numpy Mandelbrot")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("out/s01_numpy.zarr"),
    ...
    print(
        f"stage 01 numpy: center=({args.center_re}, {args.center_im}) "
