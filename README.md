...
# Stage 09: tile server over precomputed Zarrs (CPU-only)
uv run uvicorn stages.s09_viewer_fastapi.main:app
```
...
- [ ] **Stage 08** — Terraform GKE + Workload Identity Federation; Dagster K8s executor; icechunk repo in `gs://bucket/run.icechunk` via `IcechunkGCSIOManager`.
- [ ] **Stage 09** — FastAPI tile server over precomputed Zarrs (frame PNGs + slippy-map tiles). Pure CPU; deploys to Cloud Run and scales to zero.
